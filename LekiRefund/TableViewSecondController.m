...
- (NSMutableArray *)getDbData {
    ...
    NSEnumerator *enumFavList = [favList objectEnumerator];
    id elementFirstList;
    NSString *whereSql = [[NSString alloc] init];
    
    while(elementFirstList = [enumFavList nextObject])
    {
        whereSql = [whereSql stringByAppendingFormat:@"%@,", [elementFirstList objectAtIndex:0]];
    }
    whereSql = [whereSql substringWithRange:NSMakeRange(0, ([whereSql length]-1))];
	...
	if(sqlite3_open([dbPath UTF8String], &database) == SQLITE_OK) {
        ...
        NSString *querySQL = [[NSString alloc] initWithFormat:@"select *,(select group_concat(price_surcharge, ' lub ') from medicines m2 where m1.name=m2.name group by name) from medicines m1 where id in(%@) order by trim(name)",whereSql];  
...
- (void)searchBar:(UISearchBar *)searchBar textDidChange:(NSString *)searchText {
    results = [self performSearchingFor:searchText];
    //NSLog(@"asd %@", results);
    [tabelka reloadData];
}

-(NSMutableArray *)performSearchingFor:(NSString *)searchText {
    NSMutableArray *wyniki = [[NSMutableArray alloc] init ];
    NSMutableArray *whereToSearch = medicinesList; //[self compareFullList:medicinesList withFaves:favList];
    //NSLog(@"where tosearch, %@", whereToSearch);
    if([searchText length] == 0) {
        [self.searchBar resignFirstResponder];
        return whereToSearch;
    } else {
        for (NSArray *sObiekt in whereToSearch)
        {
...
- (BOOL)addFavList:(NSObject *)object {
...
-(void)buttonPressed:(UIButton *)sender {
    ...
    //NSLog(@"test");
    BOOL removeIt = [self addFavList:ob];
    //NSLog(@"removeIt %", removeIt);
    if(removeIt) {
        [results removeObject:idOb];
        [tabelka reloadData];
...
{    
    ...
    results = [self performSearchingFor:searchBar.text];
    [tabelka reloadData];
